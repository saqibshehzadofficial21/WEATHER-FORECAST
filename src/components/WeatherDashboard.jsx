...
export default function WeatherDashboard() {
  ...
  async function fetchWeatherForCoords(lat, lon, cityName = "Location") {
    setLoading(true);
    try {
      const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current_weather=true&hourly=temperature_2m,windspeed_10m,relativehumidity_2m&daily=temperature_2m_max,temperature_2m_min,weathercode&days=16&timezone=auto&windspeed_unit=kmh`;
      ...
      // Get current wind and humidity
      const currentWind = json.current_weather?.windspeed ?? null;
      const currentHumidity = json.hourly?.relativehumidity_2m
      ...
      const hourlyTemps = (json.hourly?.temperature_2m || [])
        .slice(0, 24)
        .map((t, i) => ({ temp: t, time: i }));
      const daily = (json.daily?.temperature_2m_max || []).map((t, i) => ({
        day: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][
          (new Date().getDay() + i) % 7
        ],
        date: json.daily?.time ? json.daily.time[i] : null,
        max: t,
      ...
      // compute conversions only if tempC is not null
      if (tempC != null) {
        if (unit === "F") {
          ...
          data.hourly = data.hourly.map((h) => ({
            ...h,
            temp: Math.round(cToF(h.temp) * 10) / 10,
          }));
          data.daily = data.daily.map((d) => ({
            ...d,
            max: Math.round(cToF(d.max) * 10) / 10,
            min: Math.round(cToF(d.min) * 10) / 10,
          }));
        } else if (unit === "K") {
          ...
          data.hourly = data.hourly.map((h) => ({
            ...h,
            temp: Math.round(cToK(h.temp) * 10) / 10,
          }));
          data.daily = data.daily.map((d) => ({
            ...d,
            max: Math.round(cToK(d.max) * 10) / 10,
            min: Math.round(cToK(d.min) * 10) / 10,
          }));
        } else {
      ...
      <div className="controls">
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <div className="status">
            <strong>Auto-refresh:</strong> {refreshTime / 1000}s
          </div>
